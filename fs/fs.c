...
file_block_walk(struct File *f, uint32_t filebno, uint32_t **ppdiskbno, bool alloc)
{
	...
	// indirect
		...
		if(f->f_indirect == 0) {
			...
			uint32_t indirect_filebno = filebno - NDIRECT;
...
file_get_block(struct File *f, uint32_t filebno, char **blk)
{
	...
	int result_walk = file_block_walk(f, filebno, &ppdiskbno, 1);
	...
	if(*ppdiskbno == 0) {
		int result_alloc = alloc_block();
		if(result_alloc < 0) {
			return -E_NO_DISK;
		}

		memset(diskaddr(result_alloc), 0, BLKSIZE);
		*ppdiskbno = result_alloc;
	}
