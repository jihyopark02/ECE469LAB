...
copy_shared_pages(envid_t child)
{
	...
	uint32_t upper = UTOP >> PGSHIFT;
	int inbound = 1;

	for (int npde = 0; npde < NPDENTRIES && inbound; ++npde) {
		if ((uvpd[npde] & PTE_P) == 0) {
			continue;
		}

		for (int npte = 0; npte < NPTENTRIES && inbound; ++npte) {
			uint32_t pn = NPTENTRIES * npde + npte;
			pte_t pte = uvpt[pn];
